...
class SignalRService: SignalRServiceProtocol, ObservableObject {
    ...
    /// Store a message with media attachments via SignalR
    /// - Parameters:
    ///   - conversationId: The conversation ID as UUID
    ///   - userId: The sender's user ID as UUID  
    ///   - content: Message content
    ...
    /// - Returns: Stored message
    func storeMessageWithMedia(conversationId: UUID, userId: UUID, content: String, attachments: [MediaAttachmentDto]) async throws -> SignalRMessageDto? {
        return try await invoke("StoreMessageWithMedia", parameters: [conversationId.uuidString, userId.uuidString, content, attachments])
    }
